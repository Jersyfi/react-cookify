...
import { ConsentDetailBodyProps, TableTypesType } from '../../../../types'
import { useCookifyProvider } from '../../../../context/cookifyContext'
import Collapsible from './collapsible'

export const Body: React.FC<ConsentDetailBodyProps> = ({
	desc,
	table,
	reference
}) => {
			...
			<div className="grid gap-6">
				...
				{table.types.length > 0 && (
					<div className="grid gap-3">
						{table.types.map((type: TableTypesType, index: number) => (
							<Collapsible
								key={index}
								type={type}
				...
				{typeof reference == 'object' && (
					<div className="border-2 border-[var(--c-ref-border-color)] rounded-[var(--c-ref-border-radius)] p-3">
						...
						<p>
							<b>{reference.uuid}:</b> {consentObject.uuid}
						</p>
						<p>
							<b>{reference.accepted}:</b> {at.created}
						</p>
						<p>
							<b>{reference.updated}:</b> {at.updated}
						</p>
					</div>
				)}
			</div>
