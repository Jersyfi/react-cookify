...
export const Consent: React.FC<ConsentProps> = ({ consent }) => {
	...
	const _this = {
		...
		theme: ['light', 'dark', 'high-contrast', 'custom'].includes(
			consent?.theme || ''
		)
			? consent?.theme
			: 'light',
		...
		detail: {
			...
			reference: () => {
				...
				if (
					(typeof reference == 'object' || reference == true) &&
					consent?.detail?.reference !== false
				) {
					return {
						desc:
							consent?.detail?.reference?.desc ||
							'Please provide the below information when you hand in a request about cookies.',
	...
	return (
		<div id="cookify" className={'cookify-theme-' + _this.theme}>
			{_this.paused.url !== 'undefined' &&
			_this.paused.url == pageURL &&
			consentObject.viewed === false ? (
				<Pause
